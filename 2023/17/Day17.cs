...
class Day17{
    ...
    static void Part1(){
        ...
        //Node? endNode = Nodes.Find(n => n.PosX == 8 && n.PosY == 0);
        ...
        while (Nodes.Count > 0)
        {
            Nodes = Nodes.OrderBy(n => n.GCost).ToList();
            Node cNode = Nodes[0];
            Nodes.RemoveAt(0);
            cNode.PrintNode();
            ...
            if (cNode == endNode)
            {
                Console.WriteLine(endNode.GCost);
                return;
            }
            ...
            foreach (Node n in cNode.Neighbours)
            {
                ...
                int straight = GetStraightParents(cNode, n);
